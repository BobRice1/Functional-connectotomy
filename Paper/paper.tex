...
\subsection{Structural connectivity perturbation}
We next tested whether directly damaging the structural connections of the control coupling-driver regions identified by the virtual-lesioning atlas would alter the fitted coupling regime. The top 10 control drivers included left posterior cingulate cortex, bilateral medial orbitofrontal cortex, left superior medial frontal cortex, and left inferior orbitofrontal cortex, and were contrasted with 5 non-driver regions whose Part A lesioning effects were close to zero.

At baseline ($d=0$), the fitted coupling was consistently higher for the glioma target than for the control target across all tested regions, reproducing the cohort shift seen in the main analysis. For the driver set, mean fitted values were typically $K^{*}_{\mathrm{control}} \approx 34$--$35$ and $K^{*}_{\mathrm{glioma}} \approx 36$--$37$, with similar baseline values in the non-driver set. Thus, before any perturbation, the structural-connectivity perturbation framework preserved the expected separation between control and glioma coupling estimates. The full dose-response curves are shown in Fig.~\ref{fig:scpert_dose_response}.

Progressive structural damage did change the fitted coupling, but not in the hypothesised driver-specific direction. Across the mild damage range ($0 \leq d \leq 0.5$), the slope of $K^{*}_{\mathrm{control}}$ versus damage had mean $-0.752$ for driver regions and mean $-1.946$ for non-driver regions. Over the full range ($0 \leq d \leq 1$), the corresponding mean slopes were $-2.583$ for drivers and $-2.497$ for non-drivers. Thus, damaging the top coupling drivers did not produce larger or steeper coupling shifts than damaging the null regions; if anything, the mild-range effect was numerically stronger in the non-driver set. The perturbation analysis therefore did not support the prediction that Part A driver status would translate directly into heightened structural sensitivity in Part B (Fig.~\ref{fig:scpert_sensitivity}).
...
Taken together, these results indicate that direct structural damage alone was not sufficient to reproduce the elevated-coupling signature associated with glioma. The perturbation manipulation altered fitted coupling, but it did not selectively amplify the effect of the strongest functional drivers and did not generate control-to-glioma convergence. Within this implementation, the glioma shift therefore cannot be explained as a simple consequence of weakening the structural connectivity of the Part A driver regions.

\begin{figure*}[t]
    \centering
    \includegraphics[width=0.78\textwidth]{../figures/hopf model/sc perturbation/fig1_dose_response.png}
    \caption{
    Dose-response of fitted coupling under structural damage. Solid lines show $K^{*}$ values obtained when fitting to the control PLI target, and dashed lines show fits to the glioma target, for the 10 Part A driver regions and 5 non-driver comparison regions. Although fitted coupling changes with increasing damage, the trajectories do not show a consistent driver-specific amplification and the control curves do not move toward the glioma-like regime.
    }
...
The structural-connectivity perturbation analysis provided a useful test of how far this interpretation can be pushed anatomically. Although direct damage to selected regions did alter fitted coupling, it did not do so more strongly for the Part A drivers than for the non-driver comparison set, and it did not move the control fit toward the glioma-like regime. Instead, increasing damage generally moved the control and glioma fits further apart. This suggests that the functional importance of a region in the virtual-lesioning atlas does not map in a simple one-to-one way onto structural vulnerability under this perturbation scheme.
Taken together, these results extend the conclusions of Alexandersen et al.~\cite{Alexandersen2025} in two ways. First, they support the original interpretation that tumour-associated pathology shifts whole-brain dynamics toward stronger effective interregional coupling. Second, they show that this shift can be spatially refined into a region-wise atlas of candidate coupling drivers, with the clearest glioma-specific effects emerging in a limited subset of frontal and medial regions. At the same time, the negative structural-perturbation result indicates that these candidate drivers should be interpreted primarily as functional contributors to the model fit, rather than as regions whose simple anatomical weakening is sufficient on its own to reproduce the glioma signature.
...
\begin{itemize}
    \item Test more selective structural perturbation schemes rather than uniformly scaling all incoming and outgoing edges of a region.
    \item Evaluate alternative anatomical damage models that may better approximate biologically realistic lesion effects.
    \item Extend the model beyond global perturbations by allowing region-specific parameter changes in coupling or excitability.
    \item Examine whether combining structural perturbations with functional-driver information improves the ability to reproduce the glioma-like coupling shift.
\end{itemize}
...
Building on this reproduced framework, we extended the analysis with a region-wise virtual-lesioning atlas. This showed that the coupling abnormalities identified by the original connectotomy analysis are not distributed uniformly across the brain, but are driven disproportionately by a subset of regions. Moreover, the strongest lesioning effects in glioma were associated more closely with eigenvector centrality than with degree or weighted strength, suggesting that the most important coupling drivers are globally influential regions rather than simply the most locally connected nodes. A subsequent structural-perturbation test did not show that anatomically damaging these regions was sufficient to recreate the glioma-like coupling shift, indicating that the atlas captures functional contributions to model fit more clearly than a simple anatomy-only causal mechanism.
Overall, these findings support the view that glioma-related alterations in whole-brain dynamics reflect a shift in the balance between local oscillatory activity and large-scale network interactions. More broadly, they show that connectotomy-style perturbation analyses can be extended beyond predefined tumour regions to generate region-resolved maps of candidate coupling drivers, while also highlighting the need for more refined perturbation models when linking those functional drivers back to structural mechanisms.
